...
> | 学习者画像 | 实际/计划周数 | 卡点高发周 |
> |-----------|:----------:|----------|
> | 没部署过 Python 服务 | **1.4–1.6x**（7–8 周） | Week 2（FastAPI + Docker）、Week 3（Next.js 集成） |
> | 部署过 Node 服务但没碰 Python | **1.2x**（6 周） | Week 2（uvicorn/异步 ASGI 概念） |
> | 全栈经验 | **1.0x**（5–6 周） | — |
>
> **关键提醒：** 本阶段的"超时"往往不是技术问题，而是**作品集完美主义**——想把 UI 再打磨一版、想再加一个评测维度。**上线 > 完美**：一个能跑、能讲清楚的 Demo，比一个精致但永远没上线的项目求职价值高 10 倍。
>
